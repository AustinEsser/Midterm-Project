...
font: 12pt
---
https://github.com/AustinEsser/Midterm-Project.git
...
Each IMDB page records a large amount of information about each movie. We are interested in the following:
   ...
   * The average rating of the movie by IMDB users. - 1
   * The number of ratings of the movie.  - 2
   * The year the movie was released.     - 3
   * The gross revenue of the movie (US). - 4
   * The budget for the movie.            - 5
   * The movie's title.                   -
   * The movie’s genre(s).                -
...
build_data <- function(movies_link){
   ...
   for(i in 1:n){
      ...
      rating <- c(Ave_Rating,movies_average_rating(html))

      num_ratings <- c(Num_Ratings, movies_num_ratings(html))

      genres <- c(Genres, movies_genre(html))

      gross <- c(US_Gross, movies_usgross(html))

      budget <- c(Budget, movies_budget(html))

      actors <- c(Top_Billed_Actors, movies_actors(html))
...
#data <- build_data(movies_link[1:25])
glimpse(data)
full_data <- read_csv(file = "moviedata.csv")
```
...
usgross_data <- usgross_data[!rowSums(usgross_data>115817907),1]
...
new_data <- new_data %>% group_by(year) %>% summarise(mean_profit = median(gross-budget)/10^8, mean_rating = median(rating))

ggplot(new_data, aes(x = year)) + 
   geom_line(aes(y = mean_profit, color = "Median Profit In Year(In 100 millions of Dollars)"))+
   geom_line(aes(y = mean_rating, color = "Median Rating In Year"))+
   scale_colour_manual("", 
...
ggplot(new_data, aes(x = year)) + 
   geom_line(aes(y = mean_profit, color = "Median Profit In Year(In 100 millions of Dollars)"))+
   geom_line(aes(y = mean_rating-mean(mean_rating), color = "Median Rating In Year(Centered at Zero)"))+
   scale_colour_manual("", 
